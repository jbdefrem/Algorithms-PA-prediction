from arbres_fn_jeunes import Ensemble
from arbres_fn_jeunes import Noeud
from arbres_fn_jeunes import Exemple
from arbres_fn_jeunes import Feuille
import pandas as pd
   ...
   # """
    ...
    def __construire_arbre(self, ensemble):
       ...
       #s'il ne reste d'attribut à tester
        ...
        nb_etiquette = ensemble.nombre_etiquettes()
        nb_total=len(ensemble)
        ORHAP = nb_etiquette['hap']/(nb_total-nb_etiquette['hap'])/(1809/13698)
        ORpheo = nb_etiquette['ppgl']/(nb_total-nb_etiquette['ppgl'])/(285/15222)
        y=1
        for valeur in ensemble.valeurs_possibles_attribut(a_tester):
            #on crée un sous-ensemble
            sous_ensemble = ensemble.sous_ensemble_attribut(a_tester, valeur)
            ...
            if (sous_nb_total==sous_nb_etiquette['hap']) or (sous_nb_total==sous_nb_etiquette['ppgl']):
                max, etiquette_finale = 0, ""
                #on teste toutes les étiquettes possibles de l'ensemble
                for etiquette in ensemble.etiquettes_possibles():
                    sous_ensemble = ensemble.sous_ensemble_etiquette(etiquette)
                    if len(sous_ensemble) > max:
                        ...
                        groupe = ensemble.groupe
                return Feuille(etiquette_finale,nb_total, nb_etiquette,groupe)
            if ((sous_nb_etiquette['hap']/(sous_nb_total-sous_nb_etiquette['hap'])/(1377/18265) < ORHAP and ORHAP > 3) or 
                (sous_nb_etiquette['hap']/(sous_nb_total-sous_nb_etiquette['hap'])/(15609/4033) > ORHAP and ORHAP < 0.5)) :
                y=0
            if ((sous_nb_etiquette['ppgl']/(sous_nb_total-sous_nb_etiquette['ppgl'])/(285/15522) < ORpheo and ORpheo > 3) or
                (sous_nb_etiquette['ppgl']/(sous_nb_total-sous_nb_etiquette['ppgl'])/(285/15522) > ORpheo and ORpheo < 0.5)):
                y=0
        if y == 0:
            max, etiquette_finale = 0, ""
           #on teste toutes les étiquettes possibles de l'ensemble
    ...
    def __afficher_groupes(self, feuille,ret,nb_tabs=0):
       ...
       #si c'est une feuille
           ...
           #on affiche les nombres
            if feuille.ORHAP > 3 or (feuille.ORpheo > 3 and feuille.effectif_attendu_ppgl > 10):
                ...
                sorted_groupes['OR_HAP']=feuille.ORHAP
                sorted_groupes['effectif_observe_HAP']=feuille.effectif_observe_HAP
                sorted_groupes['effectif_attendu_HAP']=feuille.effectif_attendu_HAP
                sorted_groupes['OR_PPGL']=feuille.ORpheo
                sorted_groupes['effectif_observe_ppgl']=feuille.effectif_observe_ppgl
                sorted_groupes['effectif_attendu_ppgl']=feuille.effectif_attendu_ppgl
