...
# df = sorted(df)
print(df)
df.to_csv(r'/rapids/notebooks/scripts/jeanbaptiste_work/stratif/OR/OR_all_v2.csv', index = False)


df_jeunes=pd.DataFrame()
for i in range(1,100000):
    arbre = Arbre_jeunes('/rapids/notebooks/scripts/jeanbaptiste_work/stratif/hta_feat2_jeunes.csv')
    arbre.construire()
    ret=[]
    arbre.afficher_groupes(ret)
